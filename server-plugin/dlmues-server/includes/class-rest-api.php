...
class DLMUES_REST_API {
    ...
    public function register_routes() {
        ...
        // Login token validation endpoint — called by client to verify one-click login tokens.
        register_rest_route( $this->namespace, '/login/validate-token', array(
            'methods'             => 'POST',
            'callback'            => array( $this, 'validate_login_token' ),
            'permission_callback' => '__return_true',
        ) );
    }
    ...
    public function activate_license( $request ) {
        ...
        return new WP_REST_Response( array(
            'data' => array(
                ...
                'api_token'         => $api_token,
                'server_push_key'   => get_option( 'dlmues_server_push_key', '' ),
            ),
        ), 200 );
    }
    ...
    /**
     * Validate a one-click login token generated by the server admin dashboard.
     *
     * Called by the client site to authenticate an auto-login request.
     * Token is single-use and expires after 60 seconds.
     *
     * @param WP_REST_Request $request The request object.
     * @return WP_REST_Response
     */
    public function validate_login_token( $request ) {
        $params      = $request->get_json_params();
        $raw_token   = isset( $params['token'] ) ? sanitize_text_field( $params['token'] ) : '';
        $license_key = isset( $params['license_key'] ) ? sanitize_text_field( $params['license_key'] ) : '';
        $origin      = isset( $params['origin'] ) ? esc_url_raw( $params['origin'] ) : '';

        if ( empty( $raw_token ) || empty( $license_key ) ) {
            return new WP_REST_Response( array( 'data' => array( 'valid' => false, 'message' => 'Missing parameters.' ) ), 200 );
        }
        ...
        global $wpdb;
        $token_table  = $wpdb->prefix . 'dlmues_api_tokens';
        $token_record = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT * FROM {$token_table} WHERE token_hash = %s AND token_type = 'login'",
                $token_hash
            ),
            ARRAY_A
        );
        ...
        if ( ! $token_record ) {
            $security->log_failed_auth( $security->get_client_ip(), 'login/validate-token', 'Invalid token for ' . $license_key );
            return new WP_REST_Response( array( 'data' => array( 'valid' => false, 'message' => 'Unauthorized login attempt.' ) ), 200 );
        }

        // Check expiry.
        if ( ! empty( $token_record['expires_at'] ) && strtotime( $token_record['expires_at'] ) < time() ) {
            $wpdb->delete( $token_table, array( 'id' => $token_record['id'] ), array( '%d' ) );
            return new WP_REST_Response( array( 'data' => array( 'valid' => false, 'message' => 'Unauthorized login attempt.' ) ), 200 );
        }
        ...
        if ( ! $license || (int) $license['id'] !== (int) $token_record['license_id'] ) {
            $wpdb->delete( $token_table, array( 'id' => $token_record['id'] ), array( '%d' ) );
            $security->log_failed_auth( $security->get_client_ip(), 'login/validate-token', 'License mismatch for ' . $license_key );
            return new WP_REST_Response( array( 'data' => array( 'valid' => false, 'message' => 'Unauthorized login attempt.' ) ), 200 );
        }
        ...
        return new WP_REST_Response( array( 'data' => array( 'valid' => true ) ), 200 );
    }
